...
## Результаты работы
Результат это список пар соединенных вершин, у которых известны координаты центров на изображении и на основе которых строятся изображения справа.
...
P.s. Tracker* - собственное название придуманного алгоритма.
...
### Вывод
Ни один из предложенных алгоритмов не подходит, поэтому был создан свой Tracker, который пробегается по пикселям линии. По этой причине поиск начал линий было решено как пересечения вспомогательных квадратов на основе размеров квадрата Хаара с пикселями линии графа - линий движения умного ползунка (Tracker).
...
Функция прямой: y=kx+b, следовательно: искомый коэффициент: k=(y-b)/x
Используя координаты вершин красного прямоугольника и координату начала линии (в случае ее отсутсвия – последнюю красную окружность) определяем границы значения коэффициента k для прямой, которая должна попасть в область, где находится вершина.
