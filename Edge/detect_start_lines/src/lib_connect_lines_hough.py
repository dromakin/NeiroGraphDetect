...
def drawLines(image, merged_lines_all):
    img_merged_lines = image.copy()
    for line in merged_lines_all:
        cv2.line(img_merged_lines, (line[0][0], line[0][1]), (line[1][0], line[1][1]),
                                                                                (0, 0, 255), 6)
    cv2.imwrite(folder + 'merged_lines.jpg', img_merged_lines)
...
def process_lines(orig, image, save=False):
    img = image.copy()      # cv2.imread(image_src)
    print(img.shape)
    ...
    cv2.imwrite(folder + 'tracker_roads.jpg', img)
    #
    ...
    #
    # cv2.imwrite(folder + 'lines_lines.jpg', img)

    for line in get_lines(lines):
        leftx, boty, rightx, topy = line
        cv2.line(orig_, (leftx, boty), (rightx,topy), (0, 0, 255), 6)

    cv2.imwrite(folder + 'lines_lines.jpg', orig_)
    ...
    if save == True:
        img_merged_lines = img.copy()
        for line in merged_lines_all:
            cv2.line(img_merged_lines, (line[0][0], line[0][1]), (line[1][0],line[1][1]), (0,0,255), 6)

        # cv2.imwrite(folder + 'lines_gray.jpg',gray)
        # cv2.imwrite(folder + 'lines_thresh.jpg',thresh1)
        # cv2.imwrite(folder + 'lines_edges.jpg',edges)
        cv2.imwrite(folder + 'lines_lines.jpg',img)
        cv2.imwrite(folder + 'merged_lines.jpg',img_merged_lines)
